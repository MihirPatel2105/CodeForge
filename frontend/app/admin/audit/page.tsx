...
export default function AdminAuditPage() {
  ...
  return <AdminShell>
    <AdminPageHeader eyebrow="accountability" title="Audit log" description="Filter and export the durable record of sensitive administrator actions." actions={<div className="flex gap-2"><Button variant="outline" className="h-10 gap-2 rounded-[3px]" onClick={() => downloadAdminCsv("audit-log")}><Download className="h-4 w-4" aria-hidden />Export CSV</Button><Button variant="outline" className="h-10 gap-2 rounded-[3px]" onClick={() => load(debouncedFilters, pagination?.page ?? 1)} disabled={loading}><RefreshCw className={cn("h-4 w-4", loading && "animate-spin")} aria-hidden />Refresh</Button></div>} />
