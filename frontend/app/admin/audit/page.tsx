      ...
      <AdminPageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" className="h-10 gap-2 rounded-lg" onClick={() => downloadAdminCsv("audit-log")}>
              <Download className="h-4 w-4" aria-hidden />Export CSV
            </Button>
            <Button variant="outline" className="h-10 gap-2 rounded-lg" onClick={() => load(debouncedFilters, pagination?.page ?? 1)} disabled={loading}>
      ...
      <section className="mt-6 rounded-xl border border-border bg-surface p-4 sm:p-5" aria-label="Audit log filters">
        <div className="grid gap-4 md:grid-cols-[minmax(220px,1fr)_minmax(150px,190px)_minmax(150px,190px)]">
          <label className="block min-w-0">
            ...
            <Input value={action} onChange={(event) => setAction(event.target.value)} placeholder="e.g. user.suspended" className="mt-2 h-10 rounded-lg" />
          </label>
          <label className="block min-w-0">
            <span className={ADMIN_LABEL}>From date</span>
            <Input type="date" value={from} onChange={(event) => setFrom(event.target.value)} className="mt-2 h-10 rounded-lg" />
          </label>
          <label className="block min-w-0">
            <span className={ADMIN_LABEL}>To date</span>
            <Input type="date" value={to} onChange={(event) => setTo(event.target.value)} className="mt-2 h-10 rounded-lg" />
          </label>
        </div>
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-rule pt-4">
          ...
          {hasFilters ? <Button variant="ghost" className="h-8 rounded-lg text-[12px]" onClick={clearFilters}>Clear filters</Button> : null}
        </div>
      </section>
            ...
            <tbody>
              {loading && entries.length === 0 ? Array.from({ length: 4 }, (_, index) => (
                <tr key={index} className="border-b border-rule last:border-b-0" aria-hidden>
                  {COLUMNS.map((column) => <td key={column} className="px-4 py-5"><div className="h-3 w-3/4 rounded-lg bg-surface-2" /></td>)}
                </tr>
              )) : entries.map((entry) => (
                <tr key={entry.id} className="border-b border-rule align-top transition-colors last:border-b-0 hover:bg-accent-soft/25">
                  <td className="py-4 pr-4 pl-5">
                    <span className="inline-flex max-w-[220px] items-center rounded-lg border border-accent-bd bg-accent-soft px-2.5 py-1 font-mono text-[11px] font-[700] leading-4 text-accent break-all">
                      {entry.action}
                    </span>
                  </td>
              ...
              {entries.length === 0 && !loading ? (
                <tr><td colSpan={5}>
                  <div className="flex flex-col items-center px-5 py-14 text-center">
                    <span className="flex h-10 w-10 items-center justify-center rounded-lg border border-border bg-surface-2 text-fg-muted"><SearchX className="h-5 w-5" aria-hidden /></span>
                    ...
                    {hasFilters ? <Button variant="outline" className="mt-4 rounded-lg" onClick={clearFilters}>Clear filters</Button> : null}
                  </div>
                </td></tr>
              ) : null}
            </tbody>
