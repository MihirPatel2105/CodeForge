...
export default function AdminUsersPage() {
  ...
  const load = useCallback(async (search = "", page = 1) => {
    ...
    try { const result = await api.adminUsers({ q: search, date_from: from || undefined, date_to: to ? `${to}T23:59:59Z` : undefined, page }); if (requestId !== requestSequence.current) return; setUsers(result.items); setPagination(result.pagination); }
    catch (err) {
  ...
  return (
    <AdminShell>
      <AdminPageHeader eyebrow="access and support" title="Users" description="Find an account, verify access, apply limits, and resolve compromised sessions with audited actions." actions={<div className="flex gap-2"><Button variant="outline" className="h-10 gap-2 rounded-[3px]" onClick={() => downloadAdminCsv("users")}><Download className="h-4 w-4" aria-hidden />Export CSV</Button><Button variant="outline" className="h-10 gap-2 rounded-[3px]" onClick={() => load(debouncedQuery, pagination?.page ?? 1)} disabled={loading}><RefreshCw className={cn("h-4 w-4", loading && "animate-spin")} aria-hidden />Refresh</Button></div>} />
