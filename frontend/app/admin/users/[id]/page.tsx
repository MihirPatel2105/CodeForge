...
export default function AdminUserPage() {
  ...
  return (
    <AdminShell>
      ...
      <AdminPageHeader eyebrow={user.is_admin ? "administrator account" : user.is_suspended ? "suspended account" : "user account"} title={displayName} description={user.email} actions={!user.is_admin ? <div className="flex flex-wrap gap-2"><Button variant="outline" className="h-10 gap-2 rounded-[3px]" onClick={() => setShowAction("revoke")}><KeyRound className="h-4 w-4" aria-hidden />Revoke sessions</Button>{!user.email_verified ? <Button variant="outline" className="h-10 gap-2 rounded-[3px] text-ok" onClick={() => setShowAction("verify")}><BadgeCheck className="h-4 w-4" aria-hidden />Verify email</Button> : null}<Button variant="outline" className="h-10 gap-2 rounded-[3px] border-danger-bd text-danger" onClick={() => setShowAction(user.is_suspended ? "restore" : "suspend")}>{user.is_suspended ? <RotateCcw className="h-4 w-4" aria-hidden /> : <Ban className="h-4 w-4" aria-hidden />}{user.is_suspended ? "Restore account" : "Suspend account"}</Button><Button variant="outline" className="h-10 gap-2 rounded-[3px] border-danger-bd bg-danger-soft text-danger hover:bg-danger hover:text-white" onClick={() => setShowAction("delete")}><Trash2 className="h-4 w-4" aria-hidden />Delete user</Button></div> : undefined} />
      ...
      {showAction ? <section className="mt-6 rounded-[6px] border border-danger-bd bg-danger-soft p-5" aria-labelledby="user-action-heading"><div className="flex gap-3"><ShieldAlert className="mt-0.5 h-5 w-5 shrink-0 text-danger" aria-hidden /><div className="flex-1"><h2 id="user-action-heading" className="text-[14px] font-[700] capitalize text-fg">{showAction} user account</h2><p className="mt-1 text-[12px] leading-5 text-fg-muted">{showAction === "delete" ? `This permanently removes ${user.email}, every project, run, and generated artifact. This cannot be undone.` : "This sensitive action requires a reason and is written to the audit log."}</p><Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Reason for this action" className="mt-4 h-10 rounded-[3px] bg-surface" maxLength={240} />{showAction === "delete" ? <div className="mt-3 grid gap-3 md:grid-cols-2"><Input type="password" autoComplete="current-password" aria-label="Administrator password" value={adminPassword} onChange={(event) => setAdminPassword(event.target.value)} placeholder="Administrator password" className="h-10 rounded-[3px] bg-surface" /><Input autoComplete="off" aria-label={`Type ${DELETE_CONFIRMATION} to confirm`} value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} placeholder={`Type ${DELETE_CONFIRMATION} to confirm`} className="h-10 rounded-[3px] bg-surface font-mono" /></div> : null}<div className="mt-3 flex gap-2"><Button className="h-9 rounded-[3px] bg-danger text-white hover:bg-danger/90" disabled={busy || (showAction === "delete" && (!adminPassword || deleteConfirmation.trim() !== DELETE_CONFIRMATION))} onClick={runAction}>{busy ? "Working…" : showAction === "delete" ? "Permanently delete user" : "Confirm action"}</Button><Button variant="ghost" className="h-9 rounded-[3px]" disabled={busy} onClick={() => setShowAction(null)}>Cancel</Button></div></div></div></section> : null}

      <dl className="mt-7 grid overflow-hidden rounded-[6px] border border-border bg-surface sm:grid-cols-2 lg:grid-cols-4">{[{ label: "Projects", value: user.project_count }, { label: "Runs", value: user.run_count }, { label: "Succeeded", value: user.succeeded_runs }, { label: "Joined", value: formatWhen(user.created_at) }].map(({ label, value }, index) => <div key={label} className={cn("p-5", index > 0 && "sm:border-l")}><dt className={ADMIN_LABEL}>{label}</dt><dd className="font-display mt-4 text-[21px] font-[650] tracking-[-0.04em] text-fg">{value}</dd></div>)}</dl>

      {!user.is_admin ? <section className="mt-7 rounded-[6px] border border-border bg-surface p-5"><div className="flex items-center gap-3"><Gauge className="h-5 w-5 text-accent" aria-hidden /><div><h2 className="text-[14px] font-[700] text-fg">Usage limits</h2><p className="text-[12px] text-fg-muted">Leave a field empty for unlimited access.</p></div></div><div className="mt-4 grid gap-3 md:grid-cols-3"><Input type="number" min="1" value={projectLimit} onChange={(event) => setProjectLimit(event.target.value)} placeholder={user.project_limit ? `Projects: ${user.project_limit}` : "Unlimited projects"} /><Input type="number" min="1" value={runLimit} onChange={(event) => setRunLimit(event.target.value)} placeholder={user.monthly_run_limit ? `Monthly runs: ${user.monthly_run_limit}` : "Unlimited monthly runs"} /><Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Reason for limit change" /></div><Button className="mt-3" disabled={busy} onClick={saveLimits}>Save limits</Button></section> : null}

      <section className="pt-10" aria-labelledby="projects-heading"><AdminSectionHeading eyebrow="01 / ownership" title="Projects" id="projects-heading" /><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{detail.projects.map((project) => <div key={project.id} className="rounded-[6px] border border-border bg-surface p-5"><div className="flex items-center justify-between"><FolderKanban className="h-4 w-4 text-accent" aria-hidden /><span className={ADMIN_LABEL}>{project.run_count} runs</span></div><h3 className="mt-4 text-[14px] font-[700] text-fg">{project.name}</h3><p className="mt-2 line-clamp-2 text-[12px] leading-5 text-fg-muted">{project.description || "No description"}</p></div>)}{detail.projects.length === 0 ? <p className="text-[13px] text-fg-faint">No projects yet.</p> : null}</div></section>
