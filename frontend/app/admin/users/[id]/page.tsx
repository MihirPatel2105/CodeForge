...
import { ArrowLeft, BadgeCheck, Ban, FolderKanban, Gauge, KeyRound, RefreshCw, RotateCcw, ShieldAlert } from "lucide-react";
...
export default function AdminUserPage() {
  ...
  const [showAction, setShowAction] = useState<"revoke" | "suspend" | "restore" | "verify" | null>(null);
  ...
  return (
    <AdminShell>
      ...
      {showAction ? <section className="mt-6 rounded-[6px] border border-danger-bd bg-danger-soft p-5" aria-labelledby="user-action-heading"><div className="flex gap-3"><ShieldAlert className="mt-0.5 h-5 w-5 shrink-0 text-danger" aria-hidden /><div className="flex-1"><h2 id="user-action-heading" className="text-[14px] font-[700] capitalize text-fg">{showAction} user account</h2><p className="mt-1 text-[12px] leading-5 text-fg-muted">This sensitive action requires a reason and is written to the audit log.</p><Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Reason for this action" className="mt-4 h-10 rounded-[3px] bg-surface" maxLength={240} /><div className="mt-3 flex gap-2"><Button className="h-9 rounded-[3px] bg-danger text-white hover:bg-danger/90" disabled={busy} onClick={runAction}>{busy ? "Working…" : "Confirm action"}</Button><Button variant="ghost" className="h-9 rounded-[3px]" onClick={() => setShowAction(null)}>Cancel</Button></div></div></div></section> : null}
