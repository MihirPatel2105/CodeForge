...
export default function AdminQualityPage() {
  ...
  return (
    <AdminShell>
      ...
      <section className="pt-12" aria-labelledby="rag-heading"><AdminSectionHeading eyebrow="05 / retrieval" title="RAG comparison" detail="Side-by-side only; this does not prove that RAG caused the difference." id="rag-heading" /><div className="mt-5 grid gap-4 lg:grid-cols-2">{(quality?.rag_comparison ?? []).map((group) => <div key={String(group.rag_enabled)} className="rounded-[6px] border border-border bg-surface p-5"><div className="flex items-center justify-between"><h3 className="text-[15px] font-[700] text-fg">RAG {group.rag_enabled ? "enabled" : "disabled"}</h3><span className={ADMIN_LABEL}>{group.runs} eligible runs</span></div><dl className="mt-5 grid grid-cols-2 gap-4"><Mini label="L5 rate" value={`${group.l5_rate}%`} /><Mini label="Generation" value={`${group.generation_success_rate}%`} /><Mini label="Test pass ratio" value={`${group.average_test_pass_ratio}%`} /><Mini label="Avg. iterations" value={String(group.average_iterations)} /><Mini label="Avg. duration" value={formatElapsed(group.average_duration_ms)} /></dl></div>)}</div></section>
    </AdminShell>
  );
}
...
function Kpis({ quality, loading }: { quality: AdminQualityResponse | null; loading: boolean }) {
  ...
  return <dl className="mt-5 grid overflow-hidden rounded-[6px] border border-border bg-surface sm:grid-cols-2 xl:grid-cols-4">{metrics.map(({ label, value, icon: Icon }, index) => <div key={label} className={cn("p-5", index % 2 === 1 && "sm:border-l", index > 1 && "sm:border-t", index < 4 && "xl:border-t-0", index % 4 !== 0 && "xl:border-l", index % 4 === 0 && "xl:border-l-0", index > 3 && "xl:border-t")}><div className="flex items-center justify-between"><dt className={ADMIN_LABEL}>{label}</dt><Icon className="h-4 w-4 text-fg-faint" aria-hidden /></div><dd className="font-display mt-5 text-[25px] font-[650] tracking-[-0.045em] text-fg">{loading ? "—" : value}</dd></div>)}</dl>;
}

function Breakdown({ title, eyebrow, items, tone }: { title: string; eyebrow: string; items: AdminBreakdownItem[]; tone: string }) {
  return <div><AdminSectionHeading eyebrow={eyebrow} title={title} id={title.toLowerCase().replaceAll(" ", "-")} /><div className="mt-5 space-y-4 rounded-[6px] border border-border bg-surface p-5">{items.map((item) => <div key={item.label}><div className="flex items-center justify-between gap-4"><span className="truncate font-mono text-[10px] uppercase tracking-[0.08em] text-fg-muted">{item.label.replaceAll("_", " ")}</span><span className="font-mono text-[10px] text-fg-faint">{item.count} · {item.percentage}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-surface-3"><div className={cn("h-full rounded-full", tone)} style={{ width: `${Math.min(item.percentage, 100)}%` }} /></div></div>)}{items.length === 0 ? <p className="py-5 text-center text-[12px] text-fg-faint">No data in this category.</p> : null}</div></div>;
}
