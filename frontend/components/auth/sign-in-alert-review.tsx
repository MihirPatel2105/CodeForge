...
export function SignInAlertReview({ token }: { token: string }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-bg px-5 py-14">
      <section className="w-full max-w-xl rounded-xl border border-border bg-surface p-7 shadow-[0_24px_70px_rgba(22,24,28,0.07)] md:p-10">
        ...
        {result === "not_me" ? (
          <>
            ...
            <Link href="/forgot-password" className="mt-6 inline-flex h-11 items-center rounded-lg bg-fg px-5 font-mono text-[11px] font-[700] uppercase tracking-[0.1em] text-bg">Reset password</Link>
          </>
        ) : result === "me" ? (
          <>
          ...
          <>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Button type="button" disabled={pending} onClick={() => void respond("me")} className="h-11 rounded-lg">Yes, it was me</Button>
              <Button type="button" variant="outline" disabled={pending} onClick={() => void respond("not_me")} className="h-11 rounded-lg border-danger-bd text-danger">No, sign out all devices</Button>
            </div>
